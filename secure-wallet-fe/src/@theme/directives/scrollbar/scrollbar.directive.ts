...
import { ScrollbarGeometry, ScrollbarPosition } from '@theme/directives/scrollbar/scrollbar.types';
...
export class ThemeScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    ...
    @Input() themeScrollbar: boolean = true;
    @Input() themeScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'themeScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.themeScrollbar = coerceBooleanProperty(changes.themeScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.themeScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'themeScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.themeScrollbarOptions.currentValue);
