...
import { ThemeMockApiService } from '@theme/lib/mock-api/mock-api.service';

@Injectable({
    providedIn: 'root'
})
export class ThemeMockApiInterceptor implements HttpInterceptor
{
    /**
     * Constructor
     */
    constructor(
        @Inject(THEME_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _themeMockApiService: ThemeMockApiService
    )
    {
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>>
    {
        ...
        const {
                  handler,
                  urlParams
              } = this._themeMockApiService.findHandler(request.method.toUpperCase(), request.url);
